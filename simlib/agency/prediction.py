...
class PredictionModel(metaclass=ABCMeta):
    def __init__(self, signal_inputs: Dict[str, int], signal_outputs: List[str]) -> None:
...
class PredictionModule:
    def __init__(self) -> None:
        ...
        self.models_performance: Dict[UidType, List[ModelEvaluationMetrics]] = {}

    def assign_model(self, uid: Union[int, str], model: PredictionModel) -> None:
        """Assigns a model to the specified asset uid.

        Args:
            uid (Union[int, str]): The unique identifier of the asset.
            model (PredictionModel): The model object to assign to this instance.
        """
        self.models[uid] = model

    def train_model(self, uid: Union[int, str], historical_signals: pd.DataFrame) -> None:
        """Train the model for the specified asset.

        Args:
            uid (Union[int, str]): The unique identifier of the asset.
            historical_signals (pd.DataFrame): The historical signals to train the model on.
        """
        self.models[uid].train(historical_signals)

    def get_model_prediction(
        self, uid: Union[int, str], signals_df: pd.DataFrame, new_index: Union[int, dt.datetime]
    ) -> pd.DataFrame:
        ...
        prediction_df: pd.DataFrame = self.models[uid].predict(signals_df.copy(), new_index)
